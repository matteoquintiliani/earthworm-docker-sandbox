...
NS_DOCKER_IMAGE_NAME = $(DOCKER_NAMESPACE_EXT)$(DOCKER_IMAGE_NAME)
NS_DOCKER_IMAGE_VERSION = $(NS_DOCKER_IMAGE_NAME):$(DOCKER_IMAGE_VERSION)
...
check_docker_variables:
	...
	@if [ -z "$(NS_DOCKER_IMAGE_NAME)" ]; then echo "ERROR: NS_DOCKER_IMAGE_NAME must be defined. Exit."; exit 1; fi
	@if [ -z "$(NS_DOCKER_IMAGE_VERSION)" ]; then echo "ERROR: NS_DOCKER_IMAGE_VERSION must be defined. Exit."; exit 1; fi
...
ew_run_bash: check_for_running
	docker run $(DOCKER_USER) --rm $(OPT_RUN_I) $(OPT_RUN_T) $(OPT_RUN_D) $(DOCKER_NETWORK) --name $(DOCKER_CONTAINER_NAME) $(DOCKER_PORTS) $(DOCKER_VOLUMES) $(DOCKER_ENV_COMPLETE) $(DOCKER_IMAGE_VERSION) \
...
create_tank:
	@echo $(ARGS)
	./create_tank_from_ot_lat_lon_radius/create_tank_from_ot_lat_lon_radius.sh $(DOCKER_IMAGE_VERSION) $(ARGS)
...
release: build
	docker tag $(DOCKER_IMAGE_VERSION) $(NS_DOCKER_IMAGE_VERSION)
	docker push $(NS_DOCKER_IMAGE_VERSION)
